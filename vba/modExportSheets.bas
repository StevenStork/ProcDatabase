...
' Builds export sheets from tblOperations / tblParts.
'   FFA          one sheet with every part number
'   Product Line one sheet per selected (or every) product line
'   All          FFA sheet plus every product-line sheet
' Copy-out columns:
'   Part Number | Op Sequence | Op Code | Process Hours | Avg Ex |
'   Batch Size  | Avg HPU     | Equipment Type | Home FFA | Made In FFA
...
Public Sub BuildExportSheetsCore( _
    ...
    CollectExportRowsFromStore allRows, productLineRows, (scopeIsAll Or scopeIsFfa)

    If scopeIsAll Then RemoveObsoleteExportSheets True, True, productLineRows
    If scopeIsFfa Then RemoveObsoleteExportSheets True, False, productLineRows

    If scopeIsAll Or scopeIsFfa Then
        WriteExportSheet FFA_EXPORT_SHEET_NAME, FFA_EXPORT_KEY, EXPORT_TYPE_FFA, allRows
    End If
    ...
    If scopeIsAll Or scopeIsProductLine Then
        For i = 0 To ArrayCount(productLines) - 1
            ...
            WriteExportSheet _
                BuildExportSheetName(PRODUCT_LINE_SHEET_PREFIX, keyName), _
                keyName, _
...
Private Sub CollectExportRowsFromStore( _
    ByVal allRows As Collection, _
    ByVal productLineRows As Object, _
    ByVal includeAllParts As Boolean)
    ...
    Set homeMap = HomeFfaByPart(parts)
    ...
    For rowIndex = 1 To ops.DataBodyRange.Rows.Count
        ...
        homeFfa = vbNullString
        If homeMap.Exists(partNumber) Then homeFfa = CStr(homeMap(partNumber))
        madeInFfa = Trim$(CStr(Nz(ListColumnByHeader(ops, HDR_FFA).DataBodyRange.Cells(rowIndex, 1).Value)))
        exportRow = BuildExportRow(ops, rowIndex, homeFfa, madeInFfa)

        If includeAllParts Then allRows.Add exportRow
...
Private Function HomeFfaByPart(ByVal parts As ListObject) As Object
    Dim map As Object
    Dim rowIndex As Long
    Dim basePart As String
    Dim homeCol As ListColumn

    Set map = CreateObject("Scripting.Dictionary")
    map.CompareMode = vbTextCompare
    If parts.DataBodyRange Is Nothing Then
        Set HomeFfaByPart = map
        Exit Function
    End If

    Set homeCol = ListColumnByHeader(parts, COL_PARTS_HOME_FFA)
    If homeCol Is Nothing Then Set homeCol = ListColumnByHeader(parts, COL_PARTS_FFAS_LEGACY)
    If homeCol Is Nothing Then
        Set HomeFfaByPart = map
        Exit Function
    End If
    ...
    For rowIndex = 1 To parts.DataBodyRange.Rows.Count
        ...
        If Len(basePart) > 0 Then
            If Not map.Exists(basePart) Then
                map.Add basePart, Trim$(CStr(Nz(homeCol.DataBodyRange.Cells(rowIndex, 1).Value)))
            End If
        End If
    Next rowIndex
...
Private Function BuildExportRow( _
    ByVal ops As ListObject, _
    ByVal rowIndex As Long, _
    ByVal homeFfa As String, _
    ByVal madeInFfa As String) As Variant
...
Private Sub WriteExportSheet( _
    ...
    Set ws = GetOrCreateExportSheet(sheetName, keyName, exportType)
...
Private Sub ClearExportDataArea(ByVal ws As Worksheet)
    ...
    lastRow = FastLastUsedRowInColumns(ws, "C", "L")
...
Private Sub RemoveObsoleteExportSheets( _
    ByVal rebuildFfa As Boolean, _
    ByVal rebuildProductLines As Boolean, _
    ByVal productLineRows As Object)
    ...
    For Each ws In ThisWorkbook.Worksheets
        If IsExportSheet(ws) Then
            ...
            If StrComp(exportType, EXPORT_TYPE_FFA, vbTextCompare) = 0 Then
                If rebuildFfa Then
                    keepSheet = (StrComp(keyName, FFA_EXPORT_KEY, vbTextCompare) = 0)
                Else
                    keepSheet = True
                End If
            ElseIf StrComp(exportType, EXPORT_TYPE_PRODUCT_LINE, vbTextCompare) = 0 Then
                If Not rebuildProductLines Then
                    keepSheet = True
                ElseIf productLineRows Is Nothing Then
                    keepSheet = False
                Else
